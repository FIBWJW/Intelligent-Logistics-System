<template>
  <div class="app-container">
    ...
    <el-card shadow="hover" class="chart-card">
      ...
      <div class="vehicle-pie-container" ref="vehiclePieRef"></div>
    </el-card>
  </el-col>
      <el-col :span="12">
        <el-card shadow="hover" class="chart-card">
...
export default {
  ...
  data() {
    return {
      ...
      barChart: null,
      lineChart: null,
      vehiclePieChart: null, // 新增
      vehiclePieData: [] // 用于存储车辆状态数据
    }
  },
  mounted() {
    this.initMap()
    ...
    this.initBarChart()
    this.initLineChart()
    this.fetchVehicleStatusData() // 请求车辆状态数据
  },
  beforeUnmount() {
    // 销毁所有图表实例
    ...
    if (this.vehiclePieChart) this.vehiclePieChart.dispose() // 新增
  },
  methods: {
    // 从后端接口获取车辆状态数据
    async fetchVehicleStatusData() {
      try {
        const response = await axios.get('/api/sugar/zhibiao05')
        console.log('车辆状态数据:', response.data)

        if (response.data && Array.isArray(response.data)) {
          this.vehiclePieData = response.data
          this.initVehiclePieChart() // 数据获取成功后初始化饼图
        }
      } catch (error) {
        console.error('获取车辆状态数据失败:', error)
        this.$message.error('车辆状态数据加载失败')
      }
    },

    // 初始化车辆状态饼图
    initVehiclePieChart() {
      const container = this.$refs.vehiclePieRef
      if (!container) {
        console.error('车辆状态饼图容器不存在')
        return
      }

      this.vehiclePieChart = echarts.init(container)

      // 转换后端数据格式以适应饼图显示
      const pieData = this.vehiclePieData.map(item => ({
        value: item.count,
        name: item.status
      }))
      ...
      this.vehiclePieChart.setOption(option)

      // 添加响应式调整
      window.addEventListener('resize', () => {
        this.vehiclePieChart && this.vehiclePieChart.resize()
      })
    },
